...
export function MasterReportsPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold">Reports — Assigned Users</h1>
        <p className="text-sm text-muted-foreground">
          Select an assigned user and export their purchase records
        </p>
      </div>
      ...
      {!userId ? (
        <div className="rounded border bg-card p-8 text-center text-sm text-muted-foreground">
          Select an assigned user above to view and export their purchases.
        </div>
      ) : list.isLoading ? (
        <SkeletonTable />
      ) : (
